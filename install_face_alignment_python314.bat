...
if errorlevel 1 (
    echo.
    echo [ERROR] 64-bit Python 3.14 is required.
    goto :fail
)
...
set "PIP_TEMP_DIR=%PY_DIR%pip-temp-face-alignment-314"
...
%PY% -m pip install --no-cache-dir --upgrade "psutil" "opencv-contrib-python==4.13.0.92" "mediapipe==1.0.0"
if errorlevel 1 goto :fail
...
echo Install 64-bit Python 3.14, or make sure it is available through the Python Launcher or PATH.
goto :fail
